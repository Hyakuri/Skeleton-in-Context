...
```python
{
    ...
    "checkpoint_identity_manifest_path": "<SIC_BUNDLE>/checkpoint_identity.json",
...
The public run plan must contain `method_profile.checkpoint_sha256`. SARS-Inter
copies this value from the verified identity manifest. The series runner rejects
a missing or mismatched identity before loading the model.
...
| `checkpoint_source_mode` | `identity_manifest` for formal cross-device runs; `direct_path` only for smoke diagnostics. |
| `checkpoint_identity_manifest_path` | Formal bundle manifest. The runner resolves its relative checkpoint/config filenames and verifies all hashes. |
| `checkpoint_path` | Used only by `direct_path`; its SHA256 is computed automatically. |
| `source_config` | Used only by `direct_path`; must match the selected checkpoint run. |
| `checkpoint_identity_policy` | Formal manifest mode is fixed to `require_mc_only`. `allow_legacy` is limited to direct-path old smoke checkpoints. |
