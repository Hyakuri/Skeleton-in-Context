...
```python
{
    ...
    "source_config": "<SIC_CHECKPOINT_RUN>/effective_config.yaml",
...
| `source_config` | Run-specific `effective_config.yaml` used by the checkpoint. Its content hash and MC-only task scope are included in provenance. |
| `checkpoint_identity_policy` | Use `require_mc_only` for formal runs. `allow_legacy` is limited to old smoke checkpoints and is not valid for paper results. |
